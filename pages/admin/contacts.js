...
const headers = [
  ...
  { title: 'Lido', align: 'center', render: row => !!row.checked && <i className="fas fa-check has-text-success" />}
]
...
export default function(props) {
  ...
  async function remove(id) {
    try {
      ...
      setState({
        error: true,
        message: 'Não foi possivel excluir o contato!'
      })
    }
  }
  ...
  async function load() {
    ...
    try {
      ...
      setState({ 
        loading: false,
        msgError: 'Não foi possível obter a lista de contatos!'
       })
    }
  }
      ...
      <form onSubmit={onSubmit}>            
        ...
        <div className="modal-card">
          <header className="modal-card-head">
            <p className="modal-card-title">Detalhes do contato</p>
          </header>
          <section className="modal-card-body">
            ...
            <div className="columns is-variable is-1">
              <div className="column">
                <Input type="text" label="Celular" value={data.cellphone} readOnly />
              </div>
              <div className="column">
                <Input type="email" label="Telefone fixo" value={data.phone} readOnly />
              </div>
            </div>
          ...
          <footer className="modal-card-foot">
            <Button type="submit" theme="success" icon={'fas fa-' + (data.checked ? 'undo' : 'check')}>
              {data.checked ? 'Desmarcar' : 'Marcar como lido'}
            </Button>
